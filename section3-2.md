...
**注意**：Side-set 可以让状态机在执行一条指令时，顺便设置少量 GPIO 的状态。详细描述请参见[3.5.1节](section3-5#351-side-set)。
...
- 当特定数量的比特被移出后，自动加载 OSR。参见[3.5.4节](section3-5#TODO)
- 当特定数量的比特被移入后，自动清空 ISR。参见[3.5.4节](section3-5#TODO)
...
**注意**：更紧凑的 WS2812 示例（共四条指令）参见[3.6.2节](section3-6#TODO)。
...
**注意**：Side-set（参见[3.5.1节](section3-5#TODO)）不受暂停执行的影响，永远发生在所属指令的第一个时钟周期。
